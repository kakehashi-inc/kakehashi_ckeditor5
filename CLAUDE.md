...
- Builder: Webpack 5
- Base: CKEditor 5 (v47)
- i18n: i18next (custom webpack loader)
- JS Linter: ESLint 9 (ckeditor5 config)
- JS Fixer: Prettier 3
</developper_stack_defaults>
</code_editing_rules>
...
<development_rules>
- Custom plugins MUST be placed in the `packages/` directory, each with its own `src/` and `theme/` structure.
- After every change, you MUST run `yarn build` to verify the build succeeds.
- Translation files for custom plugins MUST be placed in the `lang/` directory under the appropriate language subdirectory (e.g., `lang/ja/`, `lang/en/`).
- The build output (`build/`) is a single bundled JS file. Do not expect separate translation or CSS files.
</development_rules>
</project_details>
